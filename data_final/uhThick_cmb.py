...
def plotSchematic(r,D,bound='cmb'):
    ...
    plt.ylabel(r'$u_{h}(r)/u_{h}(r_o)$', fontsize=axfontsize)
    plt.tight_layout()
...
for i in range(len(ekDirs)):
    ...
    for j in range(len(chiDirs)):
        ...
        if ekDirs[i] == 'Ek1e-4' and chi[j] > 0.89:
            mask = d <= 2
        else:
            mask = d <= 5
        rFit = r[0] - r[mask]
        uhFit = uh_mean[mask]
        delta_cmb[i, j] = getBlayerThicknessSlope(rFit, uhFit)

        if ekDirs[i] == 'Ek1e-6' and chiDirs[j] == '0.50':
            plotSchematic(rFit, uhFit)
        print(os.getcwd())
        os.chdir('..')
...
plt.savefig('uhthick_cmbCoeff_' + plotStyle +  '.pdf',dpi=300)
